...
## API 端點
```
POST /api/v1/book          # 訂票 (user_id, ticket_type_id, quantity) → 回 202 + order_id + reserved_until + links.pay
GET  /api/v1/orders/:id    # 用 id 輪詢訂單狀態(在短暫的非同步處理視窗會回 404)
POST /api/v1/orders/:id/pay  # D4 — 為預訂建立 Stripe-shape PaymentIntent;gateway 對 order_id 冪等
...
**訂票回應契約(PR #47,D3 + D4.1 更新)。** `POST /api/v1/book` 接 `{user_id, ticket_type_id, quantity}`(D4.1:吃 `ticket_type_id`,**不是** `event_id` — KKTIX 票種模型把庫存 + 訂價放在 ticket_type 物件上),成功時回 202 Accepted,body 為 `{order_id, status: "reserved", message, reserved_until, expires_in_seconds, links: {self, pay}}`。Pattern A 的預訂語意:Redis 端的庫存「被預訂」(不再自動扣款),worker 非同步把訂單寫入資料庫,狀態 `awaiting_payment` + `reserved_until = NOW() + BOOKING_RESERVATION_WINDOW`(預設 15m),價格快照(amount_cents、currency)凍結在 `orders.amount_cents` / `orders.currency` 上。Client 在 `reserved_until` 之前 POST 到 `links.pay`(D4 — `POST /api/v1/orders/:id/pay`)以取得 `PaymentIntent`;否則 D6 的過期 sweeper 會把訂單轉為 `expired` 並回補庫存。Client 透過 `GET /api/v1/orders/:id` 輪詢最終狀態 — 在 202 之後的短暫視窗(~ms,worker 還沒寫進資料庫)會回 404,client 應該帶 backoff 重試。`order_id` 是 UUIDv7,在 API 邊界由 `BookingService.BookTicket` 鑄造,然後沿 Lua deduct → Redis stream(包含 `reserved_until` 的 unix 秒,加上 D4.1 的 `ticket_type_id` / `amount_cents` / `currency`)→ worker → 資料庫 → 輪詢一路串到底;PEL 重送會復用同一個 id。舊版的 `status: "processing"` 常數還留在 codebase 裡給 in-flight client 做向後相容,但新部署的 server 一律回 `status: "reserved"`。

**付款 intent 契約(D4 + D4.1)。** `POST /api/v1/orders/:id/pay` 成功時回 200 OK,body 為 `{order_id, payment_intent_id, client_secret, amount_cents, currency}`。Client 拿 `client_secret` 餵進 Stripe Elements(或 mock 等價物)在前端確認付款;實際的金流動作是在 D5 的 webhook(`POST /webhook/payment`)觸發時發生。Gateway 邊界冪等:對同一個 `order_id` 重複 POST 會拿到完全一樣的 PaymentIntent(gateway 把 `order_id` 當 idempotency key — Stripe 慣例)。404 表示找不到訂單;409 表示狀態不是 `awaiting_payment`(已 Paid / Expired 等)或 `reserved_until` 已過。**D4.1 訂價**:`amount_cents` 跟 `currency` 從 order 上讀(`order.AmountCents()` / `order.Currency()`)— 這個快照是 `domain.NewReservation` 在訂票當下凍結進去的(業界標準作法 — Stripe Checkout / Shopify / Eventbrite 都是訂單建立當下凍結價格)。D4.1 之前的全域變數 `BOOKING_DEFAULT_TICKET_PRICE_CENTS` + `BOOKING_DEFAULT_CURRENCY` 已移除;若部署環境還設這兩個,啟動時會印 Stderr 警告。防禦性的 `order.HasPriceSnapshot()` guard 會把任何抵達 `/pay` 但缺少快照的 legacy / migration-gap 物件,以 `ErrOrderMissingPriceSnapshot` 拒絕(對應 409、訊息獨立、handler 端記 Error 等級 log)。
...
Group ID 與 topic 名稱皆可透過 `KAFKA_SAGA_GROUP_ID` + `KAFKA_ORDER_FAILED_TOPIC` 設定。D7(2026-05-08)把舊的 `order.created` topic 跟 `KAFKA_PAYMENT_GROUP_ID` / `KAFKA_ORDER_CREATED_TOPIC` 兩個 env var,連同 `payment_worker` binary 一起刪掉了。
