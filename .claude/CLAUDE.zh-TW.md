...
**訂票回應契約(PR #47,D3 更新)。** `POST /api/v1/book` 成功時回 202 Accepted,body 為 `{order_id, status: "reserved", message, reserved_until, expires_in_seconds, links: {self, pay}}`。Pattern A 的預訂語意:Redis 端的庫存「被預訂」(不再自動扣款),worker 非同步把訂單寫入 DB,狀態 `awaiting_payment` + `reserved_until = NOW() + BOOKING_RESERVATION_WINDOW`(預設 15m)。Client 必須在 `reserved_until` 之前 POST 到 `links.pay`(D4 端點,目前回 404 — D4 才實作)才會真的扣款;否則 D6 的過期 sweeper 會把訂單轉為 `expired` 並回補庫存。Client 透過 `GET /api/v1/orders/:id` 輪詢最終狀態 — 在 202 之後的短暫視窗(~ms,worker 還沒寫進 DB)會回 404,client 應該帶 backoff 重試。`order_id` 是 UUIDv7,在 API 邊界由 `BookingService.BookTicket` 鑄造,然後沿 Lua deduct → Redis stream(包含 `reserved_until` 的 unix 秒)→ worker → DB(`orders.reserved_until` 欄位)→ 輪詢一路串到底;PEL 重送會復用同一個 id。舊版的 `status: "processing"` 常數還留在 codebase 裡給 in-flight client 做向後相容,但新部署的 server 一律回 `status: "reserved"`。
