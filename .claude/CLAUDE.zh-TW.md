...
## API 端點
```
POST /api/v1/book          # 訂票 (user_id, event_id, quantity) → 回 202 + order_id + reserved_until + links.pay
...
**訂票回應契約(PR #47,D3 更新)。** `POST /api/v1/book` 成功時回 202 Accepted,body 為 `{order_id, status: "reserved", message, reserved_until, expires_in_seconds, links: {self, pay}}`。Pattern A 的預訂語意:Redis 端的庫存「被預訂」(不再自動扣款),worker 非同步把訂單寫入 DB,狀態 `awaiting_payment` + `reserved_until = NOW() + BOOKING_RESERVATION_WINDOW`(預設 15m)。Client 在 `reserved_until` 之前 POST 到 `links.pay`(D4 — `POST /api/v1/orders/:id/pay`)以取得 `PaymentIntent`;否則 D6 的過期 sweeper 會把訂單轉為 `expired` 並回補庫存。Client 透過 `GET /api/v1/orders/:id` 輪詢最終狀態 — 在 202 之後的短暫視窗(~ms,worker 還沒寫進 DB)會回 404,client 應該帶 backoff 重試。`order_id` 是 UUIDv7,在 API 邊界由 `BookingService.BookTicket` 鑄造,然後沿 Lua deduct → Redis stream(包含 `reserved_until` 的 unix 秒)→ worker → DB(`orders.reserved_until` 欄位)→ 輪詢一路串到底;PEL 重送會復用同一個 id。舊版的 `status: "processing"` 常數還留在 codebase 裡給 in-flight client 做向後相容,但新部署的 server 一律回 `status: "reserved"`。

**付款 intent 契約(D4)。** `POST /api/v1/orders/:id/pay` 成功時回 200 OK,body 為 `{order_id, payment_intent_id, client_secret, amount_cents, currency}`。Client 拿 `client_secret` 餵進 Stripe Elements(或 mock 等價物)在前端確認付款;實際的金流動作是在 D5 的 webhook(`POST /webhook/payment`)觸發時發生。Gateway 邊界冪等:對同一個 `order_id` 重複 POST 會拿到完全一樣的 PaymentIntent(gateway 把 `order_id` 當 idempotency key — Stripe 慣例)。404 表示找不到訂單;409 表示狀態不是 `awaiting_payment`(已 Paid / Expired 等)或 `reserved_until` 已過。訂價:`amount_cents = quantity * BOOKING_DEFAULT_TICKET_PRICE_CENTS`(預設 2000),`currency = BOOKING_DEFAULT_CURRENCY`(預設 "usd")。每場活動 / 區域訂價要等 D8。
