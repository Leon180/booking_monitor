...
func NewService(repo domain.EventRepository, inventoryRepo domain.InventoryRepository, logger *mlog.Logger) Service {
	return &service{
		repo:          repo,
...
func (s *service) CreateEvent(ctx context.Context, name string, totalTickets int) (domain.Event, error) {
	...
	if err != nil {
		return domain.Event{}, fmt.Errorf("service.CreateEvent db create: %w", err)
	}
	...
	if err := s.inventoryRepo.SetInventory(ctx, created.ID(), totalTickets); err != nil {
		if delErr := s.repo.Delete(ctx, created.ID()); delErr != nil {
			...
			return domain.Event{}, fmt.Errorf("service.CreateEvent: redis SetInventory failed (%v) AND compensating DB delete failed: %w", err, delErr)
		}
		s.log.Warn(ctx, "compensated dangling event after Redis failure",
			tag.EventID(created.ID()), tag.Error(err))
		return domain.Event{}, fmt.Errorf("service.CreateEvent redis SetInventory: %w", err)
	}
