...
func reconstructFailedOrder(t *testing.T, id uuid.UUID) domain.Order {
	...
	return domain.ReconstructOrder(
		id, 1, uuid.New(), uuid.Nil, 1, domain.OrderStatusFailed, time.Now().Add(-2*time.Hour), time.Time{}, "", 0, "",
	)
}
...
func TestSweep_AlreadyCompensated_RaceWonByConsumer(t *testing.T) {
	...
	compensated := domain.ReconstructOrder(id, 1, uuid.New(), uuid.Nil, 1, domain.OrderStatusCompensated, time.Now().Add(-2*time.Hour), time.Time{}, "", 0, "")
