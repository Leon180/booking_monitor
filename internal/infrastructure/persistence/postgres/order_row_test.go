...
func TestOrderRow_FromDomain_AllFieldsCopied(t *testing.T) {
	...
	id := uuid.New()
	eventID := uuid.New()
	o := domain.ReconstructOrder(id, 7, eventID, uuid.Nil, 3, domain.OrderStatusConfirmed, createdAt, time.Time{}, "", 0, "")
...
func TestOrderRow_FromDomain_PatternA_ReservedUntilCopied(t *testing.T) {
	...
	o := domain.ReconstructOrder(uuid.New(), 7, uuid.New(), uuid.Nil, 1, domain.OrderStatusAwaitingPayment, createdAt, reservedUntil, "", 0, "")
...
func TestOrderRow_ToDomain_RoundTrip(t *testing.T) {
	...
	createdAt := time.Date(2026, 4, 25, 10, 30, 0, 0, time.UTC)
	original := domain.ReconstructOrder(uuid.New(), 7, uuid.New(), uuid.Nil, 3, domain.OrderStatusPending, createdAt, time.Time{}, "", 0, "")
...
func TestOrderRow_ToDomain_PatternA_RoundTrip(t *testing.T) {
	...
	createdAt := time.Date(2026, 4, 25, 10, 30, 0, 0, time.UTC)
	reservedUntil := createdAt.Add(15 * time.Minute)
	original := domain.ReconstructOrder(uuid.New(), 7, uuid.New(), uuid.Nil, 2, domain.OrderStatusAwaitingPayment, createdAt, reservedUntil, "", 0, "")
