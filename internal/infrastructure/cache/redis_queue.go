...
func parseMessage(msg redis.XMessage) (*worker.QueuedBookingMessage, error) {
	...
	// cases are deterministic-failure → DLQ; reject loudly at the
	// boundary so the malformed-input classifier short-circuits the
	// retry budget.
	...
	// admin / event service). Reject zero UUID for the same reason as
	// orderID: domain.NewReservation rejects it as a malformed
	// invariant, but rejecting at the boundary means the worker's
	// metrics decorator can label this "malformed_parse" instead of
	// "malformed_classified" — operationally distinct (parse failure
	// = wire-format bug; classified = domain invariant rejection).
