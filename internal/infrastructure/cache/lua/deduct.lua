...
-- ARGV[1]: count
-- ARGV[2]: user_id              (passed through to stream only)
-- ARGV[3]: order_id             (caller-minted UUIDv7; passed through to stream only)
-- ARGV[4]: reserved_until_unix  (D3 — Pattern A reservation TTL as
...
-- ARGV[5]: ticket_type_id       (passed through to stream only)
...
-- 4. Publish to Stream — order_id flows end-to-end so the worker can
