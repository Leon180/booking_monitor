...
// Integration test for compensateDanglingEvent — Stage 3's
...
//
// Mirrors PR #106's cmd/booking-cli-stage2/compensate_test.go
// because the function is duplicated across cmd binaries (see
// server.go's compensateDanglingEvent comment for the
// `package main` rationale).

func TestStage3_CompensateDanglingEvent_DeletesBothRows(t *testing.T) {
...
func TestStage3_CompensateDanglingEvent_IdempotentOnMissingRows(t *testing.T) {
	ctx := context.Background()
	pgH := pgintegration.StartPostgres(ctx, t)
