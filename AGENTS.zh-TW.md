...
POST /test/payment/confirm/:order_id  # D5(僅測試用,由 ENABLE_TEST_ENDPOINTS 控制)— 模擬 provider 端的 webhook emit;用同一把 secret 簽完後 POST 到 /webhook/payment。?outcome=succeeded|failed。
...
**Active 的 booking-hot-path 設計筆記(post-PR #90)。** 在 review 或修改 booking throughput / Redis programmability / 水平擴展相關工作前,先讀 [`docs/design/redis_runtime_metadata_scaling.zh-TW.md`](docs/design/redis_runtime_metadata_scaling.zh-TW.md)。它記錄了規劃中的 runtime-metadata follow-up(`ticket_type_meta` + `ticket_type_qty`)、為什麼 `amount_cents` 要在 reservation 時間點凍結、目前 Lua 與 Redis Functions 的取捨,以及仍然阻礙真正 Redis Cluster sharding 的 topology 限制。
