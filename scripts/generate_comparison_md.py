...
def load_intake_only_metrics(stage_dir: Path) -> dict | None:
    ...
    try:
        ...
        # Closes Slice 9 review SFH M1: corrupt summary.json (truncated,
        # disk-full, SIGKILL mid-write) is indistinguishable from
        # missing-file unless we surface it. Print to stderr so the
        # operator sees a signal rather than a silently-omitted section.
        print(
            f"WARN: {summary_path} parse failed ({e}); intake-only section "
            f"may be silently omitted from comparison.md",
            file=sys.stderr,
        )
        return None
...
def render_markdown(input_dir: Path) -> str:
    ...
    n_intake_only = sum(1 for v in intake_only_per_stage.values() if v is not None)
    has_intake_only = n_intake_only == len(STAGES)
    if 0 < n_intake_only < len(STAGES):
        # Closes Slice 9 review SFH M3: silent omission of the section
        # when 1-3 stages have intake_only_summary.json hides which
        # stage is the odd one out. Surface to stderr.
        missing_stages = [
            f"stage{n}" for n, dir_name, *_ in STAGES
            if intake_only_per_stage[n] is None
        ]
        print(
            f"WARN: {n_intake_only}/{len(STAGES)} stages have intake_only_summary.json; "
            f"missing: {', '.join(missing_stages)}. Intake-only section omitted "
            f"from comparison.md (all-or-nothing; partial section would mislead).",
            file=sys.stderr,
        )
    ...
    if has_intake_only:
        ...
        for n, _dir, label, _prose in STAGES:
            ...
            # the booking layer.
            #
            # Closes Slice 9 review SFH H1: distinguish "missing"
            # (None) from "zero" (0.0) — Python falsiness silently
            # treats both the same, but a genuine 0.0 full-flow intake
            # is an interesting data point (run was broken) that
            # deserves its own marker rather than the same "—" as
            # missing.
            ff_intake = ff.get("intake_rps")
            io_intake = io.get("intake_rps")
            if ff_intake is None or io_intake is None:
                factor_str = "—"
            elif ff_intake == 0:
                factor_str = "∞ (ff=0)"
                print(
                    f"WARN: stage {n} full-flow intake_rps is 0; "
                    f"run may have been broken (no successful bookings).",
                    file=sys.stderr,
                )
            else:
                factor_str = f"{io_intake / ff_intake:.1f}×"
            intake_rows.append(
