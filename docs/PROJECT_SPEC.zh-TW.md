...
**Race-safety。** 寫回 DB 的步驟用 SQL predicate `WHERE status = 'awaiting_payment' AND reserved_until > NOW() AND (payment_intent_id IS NULL OR payment_intent_id = $2)`。如果 D5 的 webhook 在我們的 `GetByID` 跟 `UPDATE` 之間把訂單翻成 `paid`,predicate 會 0-rows-affected;D5 引入的 3-sentinel 解析(在 `MarkPaid` 加上,並擴大到 `SetPaymentIntentID`)會把失敗精準路由:

- `ErrOrderNotFound` — row 真的消失;handler 回 404。
- `ErrReservationExpired` — `reserved_until` 在 gateway round-trip 期間已過;handler 回 409 並回「reservation expired」訊息讓 client 重新訂票。同一個 sentinel 也是 D5 webhook 處理 late-success 分支會用到的。
- `ErrInvalidTransition` — status 已被翻動(webhook / sweeper 搶先)或 row 上已經有不同的 intent_id;handler 回 409 並回「order state changed; refetch」。

把 D4 原本的「umbrella ErrOrderNotFound」擴大成 3-sentinel 合約,正是 D5 webhook 的 orphan-repair 分支能把 `SetPaymentIntentID` 失敗精準路由到 late-success 路徑的關鍵 — 不會再被誤吞成 generic 500。

### POST /webhook/payment

D5 — 入站付款 provider 的 webhook。掛在 engine root(**不**在 `/api/v1` 之下);驗證身份靠簽章,不靠網路。Envelope 是 Stripe-shape:

```json
{
  "id": "evt_3xyz",
  "type": "payment_intent.succeeded",  // 或 payment_intent.payment_failed
  "created": 1715000000,
  "livemode": false,
  "data": {
    "object": {
...
**簽章驗證。** Header 是 `Stripe-Signature: t=<unix>,v1=<hex>`,HMAC-SHA256 計算對象是 `<t>.<raw_body>`,密鑰是 `PAYMENT_WEBHOOK_SECRET`。允許 5 分鐘時鐘偏差(`PAYMENT_WEBHOOK_REPLAY_TOLERANCE`)。失敗回 401,並在 `payment_webhook_signature_invalid_total` 上記精準的 reason label。

**Order 解析。** 先主後備兩個路徑:
1. `metadata.order_id`(UUID)— `/pay` 在建立 intent 當下寫進去,是預設路徑。
2. `payment_intent_id` 反查 `orders.payment_intent_id`(migration 000015 的 partial unique idx)— 給舊有的 intent 用,或救援 `application/payment/service.go:331` 注解中的 SetPaymentIntentID race orphan。

兩個都解不到 → 回 500 + 觸發告警。**不可以**靜默 200 回不認識的 intent,因為對 orphan-rescue 來說那會把唯一的成功訊號吞掉。

**Intent-id 一致性檢查。** 走 metadata 路徑解到的 order,我們會把 row 上持久化的 `payment_intent_id` 跟 envelope 的 `object.id` 互比。不一致 → 500 + `payment_webhook_intent_mismatch_total`(單筆即 page)— 可能是 metadata 被偽造、測試 fixture 漏到 prod、或 provider 真的 bug。Handler 拒絕翻單,讓人工調查。

**狀態機分派。** 只有 `awaiting_payment` 的 order 會被處理;terminal status(`paid`/`payment_failed`/`expired`/`compensated`)直接 200 idempotent(provider 重投)。並發 webhook race 會用 `ErrInvalidTransition` 表面化,並落入相同的重讀路徑(`ProcessOrder` 在 `service.go:195-228` 已建立的範式)。

- `payment_intent.succeeded` → `MarkPaid`(race-aware SQL:`status='awaiting_payment' AND reserved_until > NOW()`)。Orphan 修復:`payment_intent_id` 為空時(race orphan),同一個 UoW 會先呼叫 `SetPaymentIntentID` 再 `MarkPaid`。
- `payment_intent.payment_failed` → `MarkPaymentFailed` + emit `order.failed`(saga compensator 回補 Redis 庫存)。
- 其他(包含 plan v5 標為 out-of-scope 的 `payment_intent.canceled`)→ 200 ACK + `payment_webhook_unsupported_type_total`。

**Late success。** `succeeded` 的 webhook 在 `reserved_until` 過期後才到,會落在 SQL predicate 的 `ErrReservationExpired`。Handler 把 order 翻成 `expired` + emit `order.failed`(saga 回補庫存)+ `payment_webhook_late_success_total{detected_at}`(`service_check` 表示 application 層的預檢就抓到、`sql_predicate` 表示資料庫端抓到極窄的 race)。**Critical 告警;需要人工去 provider 端開退款。**

**Cross-env 守門。** `livemode` 不一致(test webhook 打到 prod listener 或反過來)→ 200 no-op + `payment_webhook_unknown_intent_total{reason="cross_env_livemode"}`。避免 retry 風暴打到設錯路由的 pipeline。

### POST /test/payment/confirm/:order_id

D5 — 僅供測試,模擬 provider 端 webhook 投遞,跑完整個 pipeline。由 `cfg.Server.EnableTestEndpoints` 控制(預設關;production 部署一定保持關閉 → 整條 route 不會註冊,呼叫會回 404 而不是 401 — 不可能誤啟用)。讀取 order 已持久化的 `payment_intent_id`,組出帶 `metadata.order_id` 的 Stripe-shape envelope,用同一把 `PAYMENT_WEBHOOK_SECRET`(也就是 verifier 用的那把)簽完後,內部 POST 到 `/webhook/payment`。整合測試與 dev demo 用來在沒有真 provider 的情況下走完 pipeline。
