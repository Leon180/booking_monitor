...
| **Saga compensator — Kafka-driven hot path outcomes (D12.4)** | `saga_compensator_events_processed_total{outcome}` — counts every `order.failed` event the saga consumer processed via `Compensator.HandleOrderFailed`. Distinct from `saga_watchdog_resolved_total` — that's the DB-side sweeper; this is the Kafka consumer. **13 outcomes** (each maps to a distinct operator runbook; 4 per-step DB failures collapsed into one bullet below for readability):<br>• `compensated` = full success (UoW + Redis revert)<br>• `already_compensated` = Kafka at-least-once redelivery hitting an already-Compensated order (benign)<br>• `already_compensated_redis_error` = same as above BUT the idempotent Redis re-drive failed; SETNX guard usually makes this benign<br>• `path_c_skipped` = `resolveTicketTypeID` returned uuid.Nil (rolling-upgrade legacy event w/ multiple ticket_types). UoW + Redis both skipped. **ALERT if > 0** — inventory NOT reverted<br>• `unmarshal_error` = malformed JSON payload (consumer DLQs separately)<br>• `getbyid_error` / `list_ticket_type_error` / `incrementticket_error` / `markcompensated_error` = 4 distinct per-step DB failures inside UoW (each gets its own counter series)<br>• `redis_revert_error` = `RevertInventory` failed on FRESH compensation. PG committed but Redis qty leaked. **CRITICAL — page**<br>• `context_error` = UoW propagated `context.Canceled` / `context.DeadlineExceeded`. Usually shutdown noise<br>• `uow_infra_error` = UoW returned non-stepError (tx-begin failure, pool exhaustion). DB / pool concern<br>• `unknown` = deferred sentinel — fires only if a code path forgot `record()`. **ALERT if > 0** — code regression |
...
| `SagaCompensatorErrorRate` (D12.4) | warning | Non-success outcome ratio of `saga_compensator_events_processed_total` > 5% sustained 5m. Treats `compensated`, `already_compensated`, AND `already_compensated_redis_error` as success-equivalent (the last is a SETNX-guarded idempotent re-drive blip — Redis state was already correct from a previous delivery; including it would false-fire during Kafka redelivery bursts). Everything else (path_c_skipped, *_error labels, context_error, uow_infra_error, unknown) counts as error. False-positive note: `path_c_skipped` may dominate transiently within ~1h of a rolling upgrade (pre-v3 multi-ticket-type events flushing); see runbook for per-outcome triage. |
