...
| `SagaCompensatorErrorRate` (D12.4, updated PR-A) | warning | Non-success outcome ratio of `saga_compensator_events_processed_total` > 5% sustained 5m. Success-equivalent bucket: `compensated`, `already_compensated`, **`already_compensated_redis_mark_failed`**, **`compensated_redis_mark_failed`** (the last two are PR-A best-effort failures — Redis revert succeeded, only the `redis_reverted_at` PG write failed; inventory IS correct). Everything else (`record_completion_error`, `path_c_skipped`, `*_error`, `context_error`, `uow_infra_error`, `unknown`) counts as error — including `already_compensated_redis_error` (PR-A: PG confirmed Redis revert was needed on redelivery, the attempt failed; real inventory concern). The `*_redis_mark_failed` labels are intentionally success-equivalent; the companion `SagaMarkRedisRevertedPersistFailing` alert covers the PG write failure separately. False-positive note: `path_c_skipped` may dominate transiently within ~1h of a rolling upgrade. |
| `SagaMarkRedisRevertedPersistFailing` (PR-A) | warning | `increase(saga_mark_redis_reverted_errors_total[10m]) > 0 for 5m` — `MarkRedisReverted` PG write failed after a successful `RevertInventory`. Redis inventory IS correct; only the `redis_reverted_at` flag persistence failed. On Kafka redeliver, `revert.lua`'s EXISTS guard prevents double-revert. Sustained failures indicate PG write errors or connection pool exhaustion. Check `saga_compensator_events_processed_total{outcome=~".*redis_mark_failed"}` for context. |
