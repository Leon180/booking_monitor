...
| **Transactional outbox backlog** | `outbox_pending_count` — gauge of `events_outbox` rows where `processed_at IS NULL`, sampled at scrape time by [`OutboxPendingCollector`](../internal/infrastructure/observability/outbox_pending_collector.go). Steady-state count is single-digit (in-flight events drain within seconds via the OutboxRelay). Sustained > 100 = relay is wedged → saga compensator + payment service downstream consumers are silenced; in-flight orders cannot advance. **On scrape-time DB failure the gauge emits `0` (not stale)** — this prevents Prometheus's 5m staleness window from firing `OutboxPendingBacklog` on the previous scrape's reading during a DB outage. Truth signal during such outages is `outbox_pending_collector_errors_total` (rising counter) plus the companion `OutboxPendingCollectorDown` alert; the gauge value of 0 means "we can't tell" not "backlog cleared". Pairs with the `OutboxPendingBacklog` (warning) and `OutboxPendingCollectorDown` (critical) alerts. |
| **Inventory drift between Redis and DB (PR-D)** | `inventory_drift_detected_total{direction}` — counter, labelled `cache_missing` / `cache_high` / `cache_low_excess`. Bumped by the `recon` process's drift detector every time a sweep flags an event whose Redis qty disagrees with `event_ticket_types.available_tickets` beyond `INVENTORY_DRIFT_ABSOLUTE_TOLERANCE` (default 100).<br>• `cache_missing` = Redis returns 0 / key absent while DB has inventory → rehydrate didn't run after a Redis reset.<br>• `cache_high` = Redis > DB → P0 in flash-sale context (false 202 Accepted possible). Only causes: saga compensation desync OR manual SetInventory beyond reset baseline. PR #114 saga fence eliminated the primary double-revert path.<br>• `cache_low_excess` = Redis < DB by more than tolerance → worker is failing to commit OR recon force-fail leaks inventory.<br>Companion gauges: `inventory_drifted_events_count` (point-in-time, set per sweep), `inventory_drift_sweep_duration_seconds` (histogram), `inventory_drift_list_events_errors_total` / `inventory_drift_cache_read_errors_total` (sweep-internal failure counters). Alerts: `InventoryDriftCacheHigh` (**critical, `for: 0s`** — page immediately), `InventoryDriftCacheMissing` (warning, `for: 5m`), `InventoryDriftCacheLowExcess` (warning, `for: 5m`), `InventoryDriftListEventsErrors` (critical — DB-side blind), `InventoryDriftCacheReadErrors` (warning — partial visibility). Final piece of the cache-truth roadmap; see `docs/architectural_backlog.md` § Cache-truth architecture. |
| **DriftDetector auto-rehydrate (Gap B)** | Two counters emitted when `INVENTORY_DRIFT_AUTO_REHYDRATE=true`. **Metadata-only**: the detector calls `SetTicketTypeMetadata` (HSET) to refresh the immutable booking snapshot key (`ticket_type_meta:{id}`) for each `cache_missing` event. The qty key is intentionally NOT touched — `SET NX` from a stale DB value during the async write-behind window (in-flight deductions not yet committed) would produce `Redis qty > DB qty` → cache_high, which is P0 in flash-sale context. Qty restoration is operator-gated via startup rehydrate.<br>• `inventory_drift_auto_rehydrated_total` — metadata HSET succeeded. Operator signal: metadata key was absent; also run startup rehydrate to restore the qty key.<br>• `inventory_drift_auto_rehydrate_errors_total` — `SetTicketTypeMetadata` Redis call failed (network error, pool exhaustion). Auto-rehydrate is best-effort; `cache_missing` counter still bumps so `InventoryDriftCacheMissing` alert fires.<br>**`cache_high` is never auto-corrected**: overwriting Redis when cache > DB during the async Lua→worker write-behind window would discard in-flight deductions and worsen the drift (Redis consistency blog; Hazelcast dirty-record conflation analysis). Opt-in via env; default disabled. |
...
| **Saga compensator — Kafka-driven hot path outcomes (D12.4, updated PR-A)** | `saga_compensator_events_processed_total{outcome}` — counts every `order.failed` event the saga consumer processed via `Compensator.HandleOrderFailed`. Distinct from `saga_watchdog_resolved_total` — that's the DB-side sweeper; this is the Kafka consumer. **16 outcomes** (each maps to a distinct operator runbook; 4 per-step DB failures collapsed into one bullet below for readability):<br>• `compensated` = full success (UoW + PG fence + Redis revert)<br>• `already_compensated` = PG `saga_compensations` row confirms redis_reverted_at IS NOT NULL — fully idempotent, no Redis work done (PR-A)<br>• `already_compensated_redis_mark_failed` = wasAlreadyCompensated path, Redis revert succeeded but `MarkRedisReverted` PG write failed. Error propagated → Kafka retry; on redeliver `revert.lua` EXISTS guard (7-day TTL) is a no-op → fresh `MarkRedisReverted` attempt. Inventory IS correct during the retry window.<br>• `compensated_redis_mark_failed` = first-time compensation, Redis revert succeeded but `MarkRedisReverted` PG write failed. Error propagated → Kafka retry; same redeliver path as above. Inventory IS correct during the retry window.<br>• `record_completion_error` = `RecordCompletion` INSERT into `saga_compensations` failed inside UoW. PG MarkCompensated also rolled back — order stays Failed; watchdog will retry (PR-A)<br>• `already_compensated_redis_error` = WasRedisReverted PG check returned false (Redis revert not yet done on this redelivery), tried RevertInventory, and it failed. PG confirmed the revert was needed — **not** success-equivalent (PR-A); counts toward the error ratio in `SagaCompensatorErrorRate`<br>• `path_c_skipped` = `resolveTicketTypeID` returned uuid.Nil (rolling-upgrade legacy event w/ multiple ticket_types). UoW + Redis both skipped. **ALERT if > 0** — inventory NOT reverted<br>• `unmarshal_error` = malformed JSON payload (consumer DLQs separately)<br>• `getbyid_error` / `list_ticket_type_error` / `incrementticket_error` / `markcompensated_error` = 4 distinct per-step DB failures inside UoW (each gets its own counter series)<br>• `redis_revert_error` = `RevertInventory` failed on FRESH compensation. PG committed but Redis qty leaked. **CRITICAL — page**<br>• `context_error` = UoW propagated `context.Canceled` / `context.DeadlineExceeded`. Usually shutdown noise<br>• `uow_infra_error` = UoW returned non-stepError (tx-begin failure, pool exhaustion). DB / pool concern<br>• `unknown` = deferred sentinel — fires only if a code path forgot `record()`. **ALERT if > 0** — code regression |
| **Saga compensator — PG fence write failures (PR-A)** | `saga_mark_redis_reverted_errors_total` — increments when `MarkRedisReverted` (UPDATE saga_compensations SET redis_reverted_at = NOW()) fails after a successful `RevertInventory`. The Redis revert **did happen** — Redis inventory is correct. The compensator propagates this failure as an error so Kafka retries: on redeliver, `WasRedisReverted=false` → `RevertInventory` called → `revert.lua`'s EXISTS guard (7-day TTL) is a no-op → fresh `MarkRedisReverted` attempt. `revert.lua`'s EXISTS guard remains the last-resort defence if the retry window is exhausted (e.g. all 3 Kafka retries fail). **Sustained > 0 warrants investigation** — likely PG write errors or connection pool exhaustion. See companion outcome labels `already_compensated_redis_mark_failed` / `compensated_redis_mark_failed` for context. |
| **Saga compensator — WasRedisReverted PG check failures (PR-A)** | `saga_was_redis_reverted_errors_total` — increments when the `WasRedisReverted` SELECT on `saga_compensations` fails on a redelivery (`wasAlreadyCompensated=true`). The compensator falls open to `RevertInventory`; `revert.lua`'s EXISTS guard is the last-resort defence against double-revert. Non-zero during sustained PG degradation may produce redundant revert attempts on redelivery bursts. Companion to `saga_mark_redis_reverted_errors_total` — both signal PG health issues on the idempotency fence path. |
| **Saga compensator — end-to-end loop duration (D12.4)** | `saga_compensation_loop_duration_seconds` — histogram measuring `events_outbox.created_at → MarkCompensated commit` (threaded through `kafka.Message.Time`). Includes outbox poll delay (default 500ms), Kafka round-trip, consumer FetchMessage, compensator UoW, Redis revert. Buckets cap at 5 minutes (`{.05, .1, .25, .5, 1, 2.5, 5, 10, 30, 60, 120, 300}`) for incident-scale visibility. Observed ONLY on the `compensated` success path; `already_compensated` and `path_c_skipped` no-ops are skipped to avoid skewing p50/p99 to the floor. |
| **Saga compensator — consumer lag-since-write (D12.4)** | `saga_compensation_consumer_lag_seconds` — gauge of `time.Since(msg.Time)` for the most-recent `order.failed` message processed. **PERFORMANCE metric, not LIVENESS** — a crashed consumer leaves the gauge stale at the last value until Prometheus marks the target down (`up == 0` after `scrape_interval × 3`). Idle-reset goroutine (5s tick, 30s threshold) zeros the gauge on quiet systems so a dormant consumer doesn't false-fire alerts. Negative values from clock skew are clamped at 0. Multi-replica deployments must use `max by (instance)` to aggregate. The companion `SagaConsumerLagHigh` alert is GATED on `up == 1` to prevent false-positives during consumer crash. |
| Page funnel | `page_views_total{page}` |
| **D5 payment webhook — top-line outcomes** | `payment_webhook_received_total{result}` — every event that reaches the application service increments exactly once. `result` ∈ {`succeeded`, `failed`, `unsupported`, `unexpected_status`, `persist_failed`, `malformed`}. Hot-path throughput; alerting reads more specific counters below. |
| **D5 payment webhook — signature failures (page-worthy)** | `payment_webhook_signature_invalid_total{reason}` — verifier-side rejection counter. `reason` ∈ {`missing`, `malformed`, `skew_exceeded`, `mismatch`, `config_error`}. Sustained > 0 at any rate triggers `PaymentWebhookSignatureFailing` (warning, 5m soak). The `mismatch` reason dominates during half-done secret rotation. |
| **D5 payment webhook — unknown intent (critical)** | `payment_webhook_unknown_intent_total{reason}` — fired when neither `metadata.order_id` nor `payment_intent_id` resolved an order. `reason="not_found"` rescues the `SetPaymentIntentID` race orphan (page-worthy critical: every count is potentially "user paid but has no resolved order"). `reason="cross_env_livemode"` is benign (200 ACK, mismatched livemode). |
| **D5 payment webhook — duplicate redelivery** | `payment_webhook_duplicate_total{prior_status}` — fires when an inbound event hits a row already in a terminal state (provider redelivery) OR when concurrent webhook race falls through the `ErrInvalidTransition` re-read path. `prior_status` ∈ {`paid`, `payment_failed`, `expired`, `compensated`}. Healthy retries dominate `paid`/`payment_failed`. |
| **D5 payment webhook — intent mismatch (single-event paging)** | `payment_webhook_intent_mismatch_total` — fires when a metadata-resolved order's persisted `payment_intent_id` disagrees with the webhook envelope's `object.id`. Could be forged metadata, leaked test fixture, or a real provider bug. **Single-event paging**: the alert is `increase(...[5m]) > 0` with `for: 0m`. |
| **D5 payment webhook — late success (single-event paging)** | `payment_webhook_late_success_total{detected_at}` — fires when a `succeeded` webhook arrives after the reservation TTL elapsed. Money moved at provider but our hold is dead; the handler walks the order to `expired` + emits `order.failed` for saga compensation, **but a human MUST issue a refund**. Every count = one refund ticket. `detected_at` ∈ {`service_check`, `sql_predicate`, `post_terminal`} (the third label means the order was already terminal — D6 expired or saga compensated — when the late `succeeded` webhook arrived). |
| **D5 payment webhook — unsupported event types** | `payment_webhook_unsupported_type_total{event_type}` — counts events whose `Type` discriminator the dispatcher doesn't handle (today: anything other than `payment_intent.succeeded` / `payment_intent.payment_failed`). Lighthouse for adding future types when the value grows. |
| **D6 expiry sweeper — per-row outcomes** | `expiry_sweep_resolved_total{outcome}` — every row the sweeper processes increments exactly once. `outcome` ∈ {`expired`, `expired_overaged`, `already_terminal`, `getbyid_error`, `marshal_error`, `outbox_error`, `transition_error`}. `expired_overaged` is the labeled-but-still-transitioned signal for rows past `EXPIRY_MAX_AGE` (round-1 P1 contract: D6 always expires, MaxAge is observability-only). Pre-warmed. |
| **D6 expiry sweeper — backlog signal (page-worthy)** | `expiry_oldest_overdue_age_seconds` — gauge, set per sweep to the age (NOW − reserved_until) of the oldest still-overdue `awaiting_payment` row. Pairs with `expiry_backlog_after_sweep` (count of rows still past cutoff after the latest batch — 0 in steady state). Healthy sweepers keep oldest-age under SweepInterval + ExpiryGracePeriod + small constant ≈ 35s; > 5min sustained triggers `ExpiryOldestOverdueAge`. **Note:** on `expiry_find_expired_errors_total` failure the gauges are HELD at last-known-good (round-3 F2 contract) — readings during `ExpiryFindErrors` may pre-date the failure. |
| **D6 expiry sweeper — find / count failures (DB blind, critical)** | `expiry_find_expired_errors_total` — sweeper cannot scan OR cannot count post-sweep backlog. Sustained > 0 fires `ExpiryFindErrors` (critical). Folds two query failures into one counter because the operator response is identical (DB connectivity / index existence / pool config). Round-2 F3 + round-3 F2 design. |
| **D6 expiry sweeper — max-age awareness (single-event paging)** | `expiry_max_age_total` — increments alongside the `expired_overaged` outcome. Distinct counter (NOT a label on the resolved-vector) so the alert evaluates on a thin slice without filtering the busy resolved vector. Operator awareness only — the row IS already expired by the time this fires; the alert asks WHY it sat overdue past `EXPIRY_MAX_AGE` (default 24h) before a sweep caught it. |
| **D6 expiry sweeper — per-row vs full-sweep duration** | `expiry_resolve_duration_seconds` (per-row) + `expiry_sweep_duration_seconds` (full Sweep() wall time, NEW vs saga). Distinct semantics per round-3 F4: graph "is sweep falling behind?" (full-sweep > SweepInterval) separately from "are individual rows slow?" (per-row p95). Plus `expiry_resolve_age_seconds` — age at transition time, SLO histogram. |
| **D4.2 Stripe SDK adapter — per-call outcomes** | `stripe_api_calls_total{op,outcome}` — every outbound call to `api.stripe.com` from the StripeGateway adapter ([internal/infrastructure/payment/stripe_gateway.go](../internal/infrastructure/payment/stripe_gateway.go)) increments exactly once. `op` ∈ {`create_payment_intent`, `get_status`} (operation name without Stripe-prefix or `/v1` path); `outcome` is the classified result of the call — `success` / `declined` / `transient` / `misconfigured` / `invalid` — mapping to the `domain.ErrPayment*` sentinel taxonomy. Pre-warmed at boot via [`metrics_init.go`](../internal/infrastructure/observability/metrics_init.go) so PromQL alerts can evaluate from second 0. Operator triage by outcome: `transient` = retriable (429 / 5xx / network); `misconfigured` = page-worthy (401 / 403 — likely the API key is wrong or revoked); `declined` = card-side (becomes 422 to the booking client); `invalid` = 400 / idempotency drift (non-retryable, application bug). |
| **D4.2 Stripe SDK adapter — per-call duration** | `stripe_api_duration_seconds{op}` — histogram of wall-clock cost of one Stripe API call. Buckets: `ExponentialBuckets(0.01, 2, 14)` covering 10ms → 81.92s. Resolution at p50 (~30-50ms localhost-equivalent), p99 (~89ms per Stripe's published SLO), AND the long-tail timeouts (30s default, 5min adapter ceiling). Each retry inside stripe-go's MaxNetworkRetries loop is a separate observation, so the histogram reflects per-attempt latency, not aggregate budget. Pair with `stripe_api_calls_total` to compute outcome-stratified latency: `histogram_quantile(0.99, sum by (le, op) (rate(stripe_api_duration_seconds_bucket[5m])))`. |
...
| `SagaCompensatorErrorRate` (D12.4, updated PR-A) | warning | Non-success outcome ratio of `saga_compensator_events_processed_total` > 5% sustained 5m. Success-equivalent bucket: `compensated`, `already_compensated`, **`already_compensated_redis_mark_failed`**, **`compensated_redis_mark_failed`** (the last two trigger Kafka retry — error propagated; revert.lua EXISTS guard prevents double-revert during the retry window; treated as success-equivalent here to avoid dual-paging with `SagaMarkRedisRevertedPersistFailing`). Everything else (`record_completion_error`, `path_c_skipped`, `*_error`, `context_error`, `uow_infra_error`, `unknown`) counts as error — including `already_compensated_redis_error` (PR-A: PG confirmed Redis revert was needed on redelivery, the attempt failed; real inventory concern). False-positive note: `path_c_skipped` may dominate transiently within ~1h of a rolling upgrade. |
| `SagaMarkRedisRevertedPersistFailing` (PR-A) | warning | `increase(saga_mark_redis_reverted_errors_total[10m]) > 0 for 5m` — `MarkRedisReverted` PG write failed after a successful `RevertInventory`. Redis inventory IS correct. The compensator propagates this as an error → Kafka retries → `revert.lua` EXISTS guard (7-day TTL) is a no-op → fresh `MarkRedisReverted` attempt. If all retries fail the EXISTS guard remains the last-resort defence. Sustained failures indicate PG write errors or connection pool exhaustion. Check `saga_compensator_events_processed_total{outcome=~".*redis_mark_failed"}` for context. |
| `SagaConsumerLagHigh` (D12.4) | warning | `saga_compensation_consumer_lag_seconds > 30 for 2m` AND `up{job="booking-service"} == 1` (gated to avoid stuck-gauge false-positives during consumer crash). Lag covers outbox poll + Kafka + processing. Sustained means Kafka backlog OR slow compensator. **Forward-compat**: when PR-D12.5 lands per-stage scrape jobs and may rename `booking-service` → `d12-stage4`, this gate must be updated. **k8s caveat**: `and on(instance)` join silently produces empty result if `instance` labels diverge between gauge and `up`; replace with `on(job)` if all pods scrape under one job. |
| `SagaCompensatorClassifierDrift` (D12.4) | warning | `increase(saga_compensator_events_processed_total{outcome="unknown"}[5m]) > 0` — fires the moment the deferred-sentinel "unknown" label increments. Always a code regression: a refactor added a return path without `record()`. Identify the new return site in the latest deploy diff, add the appropriate `record(...)` call, ship a build. |
| `SagaCompensatorRedisInventoryLeak` (D12.4) | critical | `increase(saga_compensator_events_processed_total{outcome="redis_revert_error"}[5m]) > 0` — fresh-compensation Redis revert failed. PG was MarkCompensated but Redis qty NOT restored — permanent inventory leak until manual revert. **Distinct** from `already_compensated_redis_error` (PG fence confirmed Redis revert was needed on a redelivery path, the revert attempt failed — also a real inventory concern; check `SagaCompensatorErrorRate` for sustained rate). |
...
| `InventoryDriftCacheHigh` | **critical** | `increase(inventory_drift_detected_total{direction="cache_high"}[5m]) > 0 for 0s` — **P0**: Redis qty > DB qty. In a flash-sale context users may receive a false 202 Accepted for inventory that doesn't exist. Zero soak — any observation is anomalous; no "transient in-flight" explanation exists for Redis > DB. Causes: saga compensation desync (PR #114 fence eliminated the primary path) or manual SetInventory beyond reset baseline. Investigate `saga_compensations` table for double-revert patterns. |
| `InventoryDriftCacheMissing` | warning | `increase(inventory_drift_detected_total{direction="cache_missing"}[5m]) > 0 for 5m` — Redis qty key absent for ticket types with `available_tickets > 0` in DB. Lua deduct treats missing key as sold-out — customers see false sold-out. Remediate: re-run startup rehydrate (safe, idempotent). `INVENTORY_DRIFT_AUTO_REHYDRATE=true` refreshes the metadata key only; qty key still requires operator-gated startup rehydrate. |
| `InventoryDriftCacheLowExcess` | warning | `increase(inventory_drift_detected_total{direction="cache_low_excess"}[5m]) > 0 for 5m` — Redis qty < DB qty by more than `INVENTORY_DRIFT_ABSOLUTE_TOLERANCE`. Steady-state drift is positive (in-flight bookings between Lua deduct and worker commit); excess beyond tolerance means the worker is failing to commit OR the reconciler force-fail path leaks inventory (DEF-CRIT from Phase 2). Check `worker_consumer_lag` and `recon_resolved_total{outcome="max_age_exceeded"}`. |
| `InventoryDriftListEventsErrors` | critical | `rate(inventory_drift_list_events_errors_total[5m]) > 0 for 2m` — drift detector's per-sweep `ListAvailable` query is failing. While this fires the detector is BLIND on the DB side: every sweep aborts, gauge held at 0, no direction-specific alerts can fire because there's nothing to detect. Diagnose: DB outage, pool exhaustion, query timeout from the recon container. Same shape as `ReconFindStuckErrors` and `SagaWatchdogFindStuckErrors`. |
...
| `PaymentWebhookSignatureFailing` | warning | `sum(rate(payment_webhook_signature_invalid_total[5m])) > 0.1 for 5m` — D5 verifier rejecting webhooks. Look at the `reason` label split: `mismatch` after a recent rotation = secret deploy not finished; `mismatch` without rotation = potential incident; `skew_exceeded` = clock issue at provider edge; `missing` / `malformed` = probing or misrouted traffic. |
| `PaymentWebhookUnknownIntentSurging` | critical | `sum(rate(payment_webhook_unknown_intent_total{reason="not_found"}[5m])) > 0.05 for 5m` — D5 webhook couldn't resolve event to any order via either `metadata.order_id` or `payment_intent_id`. Likely cause: `SetPaymentIntentID` race in `/pay` (gateway intent created but DB write failed). Every count is potentially "user paid but has no order"; manual reconciliation per orphan. |
| `PaymentWebhookLateSuccessAfterExpiry` | critical | `increase(payment_webhook_late_success_total[5m]) > 0` — D5 single-event paging. A `succeeded` webhook arrived after `reserved_until` elapsed: money moved at provider but our hold is dead. Handler walked the order to `expired` + emitted `order.failed` for saga compensation; **a human MUST issue a refund** at the provider side. The `detected_at` label split (`service_check` vs `sql_predicate`) tells you whether `BOOKING_RESERVATION_WINDOW` is too tight (service_check dominant) or this is a narrow ms-level race (sql_predicate dominant). |
| `PaymentWebhookIntentMismatch` | critical | `increase(payment_webhook_intent_mismatch_total[5m]) > 0` — D5 single-event paging. `metadata.order_id` resolved an order whose persisted `payment_intent_id` is a DIFFERENT value from the webhook's `object.id`. Could be forged metadata, leaked test fixture, or a real provider bug. Handler refused to flip — humans MUST investigate before any auto-resolution. Treat as potential security incident. |
| `ExpiryOldestOverdueAge` | warning | `expiry_oldest_overdue_age_seconds > 300 for 5m` — D6 sweeper falling behind. Healthy sweepers keep oldest-age under SweepInterval + grace + small constant ≈ 35s; 5min sustained means rows are sitting eligible-but-unprocessed (BatchSize too small OR per-row resolves erroring). Triage `ExpiryProcessingErrors` first; bump `EXPIRY_BATCH_SIZE` if pure throughput. |
...
# InventoryDriftCacheHigh — directly mutate Redis to exceed DB value.
# Pick a known ticket_type_id from:
# `SELECT id FROM event_ticket_types WHERE available_tickets > 0 LIMIT 1`
docker exec booking_redis redis-cli SET ticket_type_qty:<ticket-type-uuid> 999999
# alert `for: 0s` — fires on the NEXT scrape (~15s). Check Prometheus → Alerts
# for direction=cache_high. No soak needed; any observation pages immediately.
# Cleanup: docker compose restart app (re-runs rehydrate) OR
# manually `SET ticket_type_qty:<uuid> <db_available_tickets>`.

# InventoryDriftCacheMissing — delete a qty key that DB still holds.
docker exec booking_redis redis-cli DEL ticket_type_qty:<ticket-type-uuid>
# Default sweep interval 60s + `for: 5m` → wait ~6m. Cleanup: docker compose restart app.

# InventoryDriftCacheLowExcess — set qty key below DB value beyond tolerance.
docker exec booking_redis redis-cli SET ticket_type_qty:<ticket-type-uuid> 0
# Same timing as CacheMissing (direction=cache_low_excess if key is present but
# drift > INVENTORY_DRIFT_ABSOLUTE_TOLERANCE). Cleanup: docker compose restart app.
