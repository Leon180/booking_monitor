...
| **Redis 與資料庫之間的庫存漂移(PR-D)** | `inventory_drift_detected_total{direction}` — counter,標籤為 `cache_missing` / `cache_high` / `cache_low_excess`。`recon` 程序中的漂移偵測器每次 sweep 發現某個事件的 Redis 數量跟 `event_ticket_types.available_tickets` 差距超過 `INVENTORY_DRIFT_ABSOLUTE_TOLERANCE`(預設 100)就 +1。<br>• `cache_missing` = Redis 回 0 / key 不存在但資料庫有庫存 → Redis 重置後 rehydrate 沒跑。<br>• `cache_high` = Redis > 資料庫 → 搶票情境下 P0(可能回傳錯誤的 202 Accepted)。唯二成因:saga 補償 desync 或手動 SetInventory 超過 reset 基線。PR #114 saga fence 已消除主要的 double-revert 路徑。<br>• `cache_low_excess` = Redis < 資料庫且超過容忍值 → worker 沒成功提交,或 recon 強制失敗洩漏庫存。<br>配套 gauge:`inventory_drifted_events_count`(point-in-time,每次 sweep 重設)、`inventory_drift_sweep_duration_seconds`(histogram)、`inventory_drift_list_events_errors_total` / `inventory_drift_cache_read_errors_total`(sweep 內部失敗 counter)。配對告警:`InventoryDriftCacheHigh`(**critical,`for: 0s`** — 立即通報)、`InventoryDriftCacheMissing`(warning,`for: 5m`)、`InventoryDriftCacheLowExcess`(warning,`for: 5m`)、`InventoryDriftListEventsErrors`(critical — 資料庫端瞎掉)、`InventoryDriftCacheReadErrors`(warning — 部分可見)。Cache-truth 路線圖的最後一塊;詳見 `docs/architectural_backlog.md` § Cache-truth architecture。 |
| **DriftDetector 自動補水(Gap B)** | 當 `INVENTORY_DRIFT_AUTO_REHYDRATE=true` 時發出的兩個 counter。**僅補 metadata**:偵測器對每個偵測到的 `cache_missing` 事件呼叫 `SetTicketTypeMetadata`(HSET),重新整理不可變的訂票快照 key(`ticket_type_meta:{id}`)。qty key 刻意不補寫——在非同步回寫視窗期間(在途 deduction 尚未提交至資料庫),以舊資料庫值執行 `SET NX` 會造成 `Redis qty > DB qty` → cache_high,在搶票情境下是 P0 問題(使用者可能收到不存在庫存的 202 Accepted)。qty key 的還原需由運維人員手動執行啟動補水。<br>• `inventory_drift_auto_rehydrated_total` — metadata HSET 成功。運營信號:metadata key 曾缺失;qty key 仍需執行啟動補水。<br>• `inventory_drift_auto_rehydrate_errors_total` — `SetTicketTypeMetadata` Redis 呼叫本身失敗(網路錯誤、連線池耗盡)。自動補水為盡力(best-effort);`cache_missing` counter 仍會 +1 以確保 `InventoryDriftCacheMissing` 告警觸發。<br>**`cache_high` 永不自動修正**:在 Lua→worker 非同步回寫視窗期間,以資料庫值覆寫 Redis 會丟棄在途的 deduction,反而加劇漂移(Redis 一致性部落格;Hazelcast dirty-record 衝突分析)。透過環境變數選擇啟用;預設停用。 |
...
| **Saga compensator — Kafka 驅動熱路徑處理結果(D12.4,PR-A 更新)** | `saga_compensator_events_processed_total{outcome}` — counter,記 saga consumer 透過 `Compensator.HandleOrderFailed` 處理的每筆 `order.failed` event。跟 `saga_watchdog_resolved_total` 不同 — 那是 DB 端的 sweeper;這條是 Kafka 消費者。**16 個 outcome**(每個對應不同 runbook;4 個 UoW 內部各步驟 DB 失敗在下方併成一個項次方便閱讀):<br>• `compensated` = 完全成功(UoW + PG fence 寫入 + Redis revert)<br>• `already_compensated` = PG `saga_compensations` 資料列確認 redis_reverted_at IS NOT NULL — 完全冪等,Redis 工作跳過(PR-A)<br>• `already_compensated_redis_mark_failed` = wasAlreadyCompensated 路徑,Redis revert 成功但 `MarkRedisReverted` PG 寫入失敗。回傳 error → Kafka 重試;重投遞時 `revert.lua` EXISTS 守門(7 天 TTL)為 no-op → 再次嘗試 `MarkRedisReverted`。重試窗口內庫存數字正確。<br>• `compensated_redis_mark_failed` = 首次補償,Redis revert 成功但 `MarkRedisReverted` PG 寫入失敗。回傳 error → Kafka 重試;重投遞路徑同上。重試窗口內庫存數字正確。<br>• `record_completion_error` = `RecordCompletion` 在 UoW 內寫入 `saga_compensations` 失敗。PG MarkCompensated 也一起回滾 — 訂單維持 Failed 狀態,watchdog 將重試(PR-A)<br>• `already_compensated_redis_error` = `WasRedisReverted` PG 查詢回傳 false（此次重投遞時 Redis revert 尚未完成），嘗試 `RevertInventory` 後失敗。PG 確認 revert 確實需要執行 — **不屬於成功等價**（PR-A）；計入 `SagaCompensatorErrorRate` 的錯誤比例<br>• `path_c_skipped` = `resolveTicketTypeID` 回傳 uuid.Nil(rolling-upgrade 期間有多 ticket_type 的舊事件)。UoW + Redis 都跳過。**> 0 即告警** — 庫存沒被回復<br>• `unmarshal_error` = JSON payload 壞掉(consumer 另外送 DLQ)<br>• `getbyid_error` / `list_ticket_type_error` / `incrementticket_error` / `markcompensated_error` = UoW 內部 4 個獨立步驟的 DB 失敗(各自獨立的 counter series)<br>• `redis_revert_error` = `RevertInventory` 在新鮮的補償流程中失敗。PG 已 commit 但 Redis qty 漏了。**Critical — 該 page**<br>• `context_error` = UoW 把 `context.Canceled` / `context.DeadlineExceeded` 傳出來。通常是 shutdown 雜訊<br>• `uow_infra_error` = UoW 回傳的不是 stepError(tx-begin 失敗、連線池耗盡)。是 DB / pool 健康度議題<br>• `unknown` = 延遲執行的 sentinel — 只在某條程式碼路徑忘了呼叫 `record()` 時觸發。**> 0 即告警** — 程式碼 regression |
| **Saga compensator — PG fence 寫入失敗(PR-A)** | `saga_mark_redis_reverted_errors_total` — 在 `RevertInventory` 成功後,`MarkRedisReverted`(UPDATE saga_compensations SET redis_reverted_at = NOW())失敗時累加。Redis revert **已發生** — Redis 庫存數字正確。compensator 將此失敗以 error 回傳,讓 Kafka 重試:重投遞時 `WasRedisReverted=false` → 呼叫 `RevertInventory` → `revert.lua` EXISTS 守門(7 天 TTL)為 no-op → 再次嘗試 `MarkRedisReverted`。若所有重試(例如 3 次 Kafka retry)全部失敗,`revert.lua` 的 EXISTS 守門仍為最後防線。**持續 > 0 需調查** — 可能是 PG 寫入錯誤或連線池耗盡。參考對應的 outcome label `already_compensated_redis_mark_failed` / `compensated_redis_mark_failed` 了解情境。 |
| **Saga compensator — WasRedisReverted PG 查詢失敗(PR-A)** | `saga_was_redis_reverted_errors_total` — 在重投遞（`wasAlreadyCompensated=true`）路徑上，`WasRedisReverted` 對 `saga_compensations` 的 SELECT 查詢失敗時累加。compensator 採取 fail-open 策略繼續執行 `RevertInventory`；`revert.lua` 的 EXISTS 守門作為防止 double-revert 的最後防線。PG 降級期間若大量重投遞同時進入，非零值可能導致多餘的 revert 嘗試。與 `saga_mark_redis_reverted_errors_total` 互為配對 — 兩者都指向冪等 fence 路徑上的 PG 健康度問題。 |
...
Admin SSE 事件管線暴露自己的基礎設施指標(Layer B,見 [§Q16](design/admin_event_streaming.md))。這些指標告訴 SRE 串流管線本身是否健康 — 跟 `bookings_total` 這類業務指標(Layer A)是兩個不同層級。

| 指標 | 類型 | Label | 用途 |
|---|---|---|---|
| `admin_event_bus_published_total` | counter | `event_type`(8 種值) | `bus.Publish()` 成功進入 channel 的事件 |
| `admin_event_bus_dropped_total` | counter | `reason`(`channel_full`) | 在發布端因背壓而丟棄的事件 |
| `admin_event_bus_xadd_failures_total` | counter | `reason`(`timeout`/`conn_refused`/`other`) | drainer goroutine 的 Redis XADD 失敗 |
| `admin_event_bus_channel_depth` | gauge | — | 即時的記憶體 channel 深度 |
| `admin_sse_active_connections` | gauge | — | 目前連線中的 admin 客戶端 |
| `admin_sse_messages_sent_total` | counter | `event_type` | 已送達客戶端的事件 |
| `admin_sse_messages_dropped_total` | counter | `reason` | hub 在 per-client 背壓時丟棄(罕見;Q10 預設改為踢掉 client) |
| `admin_sse_clients_dropped_total` | counter | `reason`(`slow_consumer`/`write_error`/`shutdown`) | 被強制斷線的 client |
| `admin_sse_reconnects_total` | counter | — | 帶 `Last-Event-ID` 的連線嘗試 |
| `admin_sse_heartbeats_sent_total` | counter | — | 30 秒一次的 comment-line 心跳 |
| `admin_sse_events_truncated_total` | counter | — | 重連時 `Last-Event-ID` 已超出 MAXLEN trim 區間 |
| `admin_sse_connection_duration_seconds` | histogram | — | Admin 分頁連線時長分佈 |
...
驅動下方 [SLO 燃燒率告警](#5-告警) 的階層式預計算 SLI。SLO 定義與錯誤預算數學請見 [docs/slo.md](slo.md)。檔案:[deploy/prometheus/recording_rules.yml](../deploy/prometheus/recording_rules.yml)。三層架構(Tier 1 → 2 → 3):

| Recording rule | 類型 | 用途 |
|---|---|---|
| `booking:requests:rate5m` / `booking:errors:rate5m` | tier 1,30s 評估 | 廉價的 5m 窗口;驅動告警的 short window |
| `booking:book_latency:fast_rate5m` / `..._total_rate5m` | tier 1,30s 評估 | 同上,但針對 `POST /api/v1/book` 在 500ms 內完成的計數 vs 總計數 |
| `booking:availability:ratio_rate{5m,30m,1h,6h,3d}` | tier 2,1m 評估 | 各告警層級窗口下的可用性成功率聚合 |
| `booking:book_latency:fast_ratio_rate{5m,30m,1h,6h,3d}` | tier 2,1m 評估 | 同上,但針對 booking latency SLO |
| `booking:availability:ratio_rate30d` | tier 3,5m 評估 | 30 天 SLI,給儀表板用 |
| `booking:availability:slo_budget_remaining` | tier 3,5m 評估 | 錯誤預算剩餘(0..1),負值代表 SLO 已破 |
| `booking:book_latency:fast_ratio_rate30d` | tier 3,5m 評估 | 30 天 booking latency SLI |
| `booking:book_latency:slo_budget_remaining` | tier 3,5m 評估 | Booking latency 的錯誤預算剩餘 |

為什麼要階層化:`rate(...[30d])` 每 30s 評估一次代表每個 tick 都要掃 30 天的原始 sample(規模大時很貴)。先聚合到 5m 窗口、再在 SLO 窗口上做平均,是 [Prometheus rules best practice](https://prometheus.io/docs/practices/rules/)。
...
**第三個預先配置的 dashboard:Admin War-Room(PR #121)**
...
| `SagaCompensatorErrorRate`(D12.4，PR-A 更新) | warning | `saga_compensator_events_processed_total` 的非成功 outcome 比例持續 5 分鐘 > 5%。成功等價桶：`compensated`、`already_compensated`、**`already_compensated_redis_mark_failed`**、**`compensated_redis_mark_failed`**（後兩者觸發 Kafka 重試 — error 回傳；`revert.lua` EXISTS 守門在重試窗口內防止 double-revert；此處列為成功等價以避免與 `SagaMarkRedisRevertedPersistFailing` 雙重告警）。其他（`record_completion_error`、`path_c_skipped`、各 `*_error`、`context_error`、`uow_infra_error`、`unknown`）才算錯誤 — 包括 `already_compensated_redis_error`（PR-A：PG 確認重投遞時 Redis revert 需要執行，嘗試後失敗；屬真實庫存隱患）。誤觸發提醒：`path_c_skipped` 在 rolling upgrade 後 ~1 小時內可能短暫佔比偏高。 |
| `SagaMarkRedisRevertedPersistFailing`(PR-A) | warning | `increase(saga_mark_redis_reverted_errors_total[10m]) > 0 for 5m` — `MarkRedisReverted` PG 寫入在 `RevertInventory` 成功後失敗。Redis 庫存**已正確**。compensator 將此失敗以 error 回傳 → Kafka 重試 → `revert.lua` EXISTS 守門(7 天 TTL)為 no-op → 再次嘗試 `MarkRedisReverted`。若所有重試全部失敗，EXISTS 守門仍為最後防線。持續觸發代表 PG 寫入錯誤或連線池耗盡。查 `saga_compensator_events_processed_total{outcome=~".*redis_mark_failed"}` 了解情境。 |
...
| `SagaCompensatorRedisInventoryLeak`(D12.4) | critical | `increase(saga_compensator_events_processed_total{outcome="redis_revert_error"}[5m]) > 0` — 新鮮的補償流程中 Redis revert 失敗。PG 已被 MarkCompensated 但 Redis qty **沒**回復 — 永久庫存漏失,需人工 revert。**跟 `already_compensated_redis_error` 不同**（PG fence 確認在重投遞路徑上 Redis revert 確實需要執行，嘗試後失敗 — 同樣是真實庫存隱患；查 `SagaCompensatorErrorRate` 持續比例）。 |
...
| `InventoryDriftCacheHigh` | **critical** | `increase(inventory_drift_detected_total{direction="cache_high"}[5m]) > 0 for 0s` — **P0**:Redis qty > 資料庫 qty。搶票情境下使用者可能收到不存在庫存的 202 Accepted。零 soak — 任何觀測值都異常,Redis > 資料庫在正常運作中不存在「短暫 in-flight」的解釋。成因:saga 補償 desync(PR #114 fence 已消除主要路徑)或手動 SetInventory 超過 reset 基線。調查 `saga_compensations` 資料表是否有 double-revert 模式。 |
| `InventoryDriftCacheMissing` | warning | `increase(inventory_drift_detected_total{direction="cache_missing"}[5m]) > 0 for 5m` — Redis qty key 不存在但資料庫仍有 `available_tickets > 0`。Lua deduct 將缺失 key 視為售完 — 使用者看到錯誤的售完提示。修復:重新執行啟動補水(安全、冪等)。`INVENTORY_DRIFT_AUTO_REHYDRATE=true` 只補 metadata key;qty key 仍需運維人員手動執行啟動補水。 |
| `InventoryDriftCacheLowExcess` | warning | `increase(inventory_drift_detected_total{direction="cache_low_excess"}[5m]) > 0 for 5m` — Redis qty < 資料庫 qty,且差距超過 `INVENTORY_DRIFT_ABSOLUTE_TOLERANCE`。穩態漂移為正(Lua deduct 到 worker 提交之間的在途訂單);超過容忍值代表 worker 沒成功提交,或 reconciler 強制失敗路徑洩漏庫存(Phase 2 DEF-CRIT)。檢查 `worker_consumer_lag` 與 `recon_resolved_total{outcome="max_age_exceeded"}`。 |
| `InventoryDriftListEventsErrors` | critical | `rate(inventory_drift_list_events_errors_total[5m]) > 0 for 2m` — 漂移偵測器每次 sweep 的 `ListAvailable` 查詢在失敗。觸發期間偵測器在資料庫端瞎掉:每次 sweep 都中斷、gauge 保持為 0、各 direction 告警也無法觸發(因為沒東西可偵測)。診斷:資料庫離線、connection pool 爆掉、recon container 端的查詢逾時。形狀跟 `ReconFindStuckErrors` 與 `SagaWatchdogFindStuckErrors` 一致。 |
...
| `AdminEventBusDropping` | warning | `rate(admin_event_bus_dropped_total[5m]) > 0 for 2m` — PR #121 admin event 發布端背壓。bus channel(cap 10,000)滿了,事件在 `Publish()` 階段被丟棄。最常見原因:drainer goroutine XADD 不夠快(Redis 慢 / down)。觀測能力下降,但業務功能不受影響。檢查 `rate(admin_event_bus_xadd_failures_total[5m])` 看 Redis 是否瓶頸;`admin_event_bus_channel_depth` 看佇列飽和度。 |
| `AdminStreamSubscriberConsecFailures` | critical | `admin_stream_subscriber_consec_failures > 10 for 1m` — PR #121 SSE subscriber goroutine 連續失敗中。這個是 lead-time 告警,在達到設定門檻 30(觸發 `fx.Shutdown(ExitCode(1))` 讓 k8s 重啟 pod)之前先通知。趁 escalation 觸發前去查 Redis 健康度 / 網路 / pod 本地資源壓力。 |
| `AdminSSEReconnectsBursting` | warning | `rate(admin_sse_reconnects_total[1m]) > 5 for 30s` — 重連速率異常升高。Deploy 後短暫升高是正常的(clients 帶 jittered `retry:` 提示重連,見 § Q15)。如果關機視窗結束後還持續飆,可能是網路不穩、LB idle-timeout 配錯,或 pod 重啟迴圈。 |
| `AdminSSEEventsTruncated` | warning | `increase(admin_sse_events_truncated_total[10m]) > 0` — 至少有一個 client 帶著比 stream MAXLEN trim 還舊的 Last-Event-ID 重連。Server 發了一個合成的 `stream_truncated` 事件,然後從 live 接上。高流量閃購期間是預期的;非高峰期出現代表 client 斷線太久,超過 ~100k entry 的保留範圍。 |
| `InventoryLowAlerts` | warning | `rate(inventory_low_alerts_total[5m]) > 0 for 1m` — 有一個以上的 ticket_type 跌破低庫存閾值(預設 10%)。Edge-triggered — 只在「穿越」發生時觸發,不會在庫存維持低水位時持續燒。要查具體哪個 ticket_type 透過 SSE 事件日誌或直接查 `event_ticket_types` 資料表。 |
| `BookingAvailabilitySLOBurnRateFast` (PR 9) | page | 1h + 5m 燃燒率 > 14.4×(= 一小時內燒掉 30 天錯誤預算的 2%)。雙窗口閘門過濾掉瞬時 burst。以這個速率,整個 30 天預算約 2.08 天燒光。來源:[SRE Workbook ch. 5 Table 5-8](https://sre.google/workbook/alerting-on-slos/)。 |
| `BookingAvailabilitySLOBurnRateMedium` (PR 9) | page | 6h + 30m 燃燒率 > 6×(= 6 小時內燒掉 30 天預算的 5%)。抓「持續但非極快」的衰退;以此速率預算約 5 天燒光。 |
| `BookingAvailabilitySLOBurnRateSlow` (PR 9) | ticket | 3d + 6h 燃燒率 > 1×(= 3 天內燒掉 30 天預算的 10%)。緩慢的爬升 — 不到 page 等級,但下個工作日要查。 |
| `BookingLatencySLOBurnRateFast` (PR 9) | page | 跟可用性 fast burn 同樣形狀,但作用在 booking-latency SLI:1h 與 5m 兩個窗口上,`POST /api/v1/book` 在 500ms 內完成的比例都 < 85.6%。SLO 目標 = 30 天內 99% 在 500ms 內完成。 |
| `BookingLatencySLOBurnRateMedium` (PR 9) | page | Booking-latency SLI 上的 6h + 30m × 6 燃燒。常見原因:上游壓力(Redis/PG 飽和、GC pause、deploy rollback 進行中)。 |
| `BookingLatencySLOBurnRateSlow` (PR 9) | ticket | Booking-latency SLI 上的 3d + 6h × 1 燃燒 — 緩慢的延遲爬升(Postgres 膨脹、Redis key 累積、可能的 goroutine 洩漏)。下個工作日查。 |
...
# InventoryDriftCacheHigh — 直接把 Redis qty 設成超過資料庫的值。
# 先從下列查詢取得 ticket_type_id:
...
# 告警 `for: 0s` — 下一次 scrape(~15 秒)就會觸發。Prometheus → Alerts 看
# direction=cache_high。零 soak,任何觀測值都立即通報。
# 清理:docker compose restart app(重新執行 rehydrate),
# 或手動 `SET ticket_type_qty:<uuid> <資料庫的 available_tickets>`。

# InventoryDriftCacheMissing — 刪除資料庫仍持有的 qty key。
docker exec booking_redis redis-cli DEL ticket_type_qty:<ticket-type-uuid>
# 預設 sweep interval 60s + `for: 5m` → 等 ~6m。清理:docker compose restart app。

# InventoryDriftCacheLowExcess — 把 qty key 設成低於資料庫值且超過容忍值。
docker exec booking_redis redis-cli SET ticket_type_qty:<ticket-type-uuid> 0
# 時間同 CacheMissing(key 存在但 drift > INVENTORY_DRIFT_ABSOLUTE_TOLERANCE 時
# direction=cache_low_excess)。清理:docker compose restart app。
