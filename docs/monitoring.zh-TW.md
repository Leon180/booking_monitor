...
| `TicketTypeCacheRedisErrors` | warning | `rate(cache_errors_total{cache="ticket_type",op=~"get\|set"}[5m]) > 0 for 5m` — ticket_type read-through cache 的 Redis 路徑出錯。Cache 是 fail-soft 的(訂票仍然透過 PG fallback 運作),但 PR #90 帶來的 +38% sold-out fast-path RPS 增益被靜默地停用。跟 `cache_misses_total` 拆開,讓 Redis 故障不會偽裝成 cache-cold spike。**severity 設 warning + 5m soak 是刻意的**:這個 cache 是效能優化、**不是**正確性控制(跟 `IdempotencyCacheGetErrors` 不同);5m 可以吸收例行的 Redis 重啟,避免 1m 警示被路面操作 false-positive。 |
| `TicketTypeCacheMarshalErrors` | warning | `rate(cache_errors_total{cache="ticket_type",op="marshal"}[5m]) > 0 for 1m` — cache entry 的 JSON marshal 失敗。對固定 shape 的 DTO 來說這是理論上不會發生的,所以任何非零值都是**程式 regression**,不是 Redis 事件。操作員處理方式:**回報 bug、不要叫 Redis on-call**。跟 Redis-error 那條 alert 拆開是因為兩者的處理動作完全不同。 |
...
# 漂移 sweep 把它抓出來。先從 `SELECT id FROM events WHERE available_tickets > 0 LIMIT 1`
# 拿到一個已知的 event_id,然後:
...
# 或手動 `SET event:<uuid>:qty <資料庫的 available_tickets>` 把值補回來。
