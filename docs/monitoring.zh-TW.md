...
| `SagaCompensatorErrorRate`(D12.4，PR-A 更新) | warning | `saga_compensator_events_processed_total` 的非成功 outcome 比例持續 5 分鐘 > 5%。成功等價桶：`compensated`、`already_compensated`、**`already_compensated_redis_mark_failed`**、**`compensated_redis_mark_failed`**（後兩者是 PR-A 的 best-effort 失敗 — Redis revert 已成功，只有 `redis_reverted_at` PG 寫入失敗；庫存狀態正確）。其他（`record_completion_error`、`path_c_skipped`、各 `*_error`、`context_error`、`uow_infra_error`、`unknown`）才算錯誤 — 包括 `already_compensated_redis_error`（PR-A：PG 確認重投遞時 Redis revert 需要執行，嘗試後失敗；屬真實庫存隱患）。`*_redis_mark_failed` 刻意列為成功等價；PG 寫入失敗由 `SagaMarkRedisRevertedPersistFailing` 另外告警。誤觸發提醒：`path_c_skipped` 在 rolling upgrade 後 ~1 小時內可能短暫佔比偏高。 |
| `SagaMarkRedisRevertedPersistFailing`(PR-A) | warning | `increase(saga_mark_redis_reverted_errors_total[10m]) > 0 for 5m` — `MarkRedisReverted` PG 寫入在 `RevertInventory` 成功後失敗。Redis 庫存**已正確**；只是 `redis_reverted_at` 旗標的持久化失敗。Kafka 重投時 `revert.lua` 的 EXISTS 守門防止 double-revert。持續觸發代表 PG 寫入錯誤或連線池耗盡。查 `saga_compensator_events_processed_total{outcome=~".*redis_mark_failed"}` 了解情境。 |
