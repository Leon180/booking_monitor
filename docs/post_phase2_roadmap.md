...
- **`v0.6.0` shipped 2026-05-08** — D7 deleted the legacy A4 auto-charge path (payment_worker binary + `order.created` consumer + `ProcessOrder`); saga scope narrowed to `{expired, payment_failed}`. D8-minimal browser demo (Vite + React + TS in `demo/`) ships against the CORS allow-list from PR #96.
- **`v1.0.0` shipped 2026-05-09 — Phase 3 closed** — D4.2 (real Stripe SDK adapter, [#110](https://github.com/Leon180/booking_monitor/pull/110)) + D9-minimal / D10-minimal ([#102](https://github.com/Leon180/booking_monitor/pull/102), [#103](https://github.com/Leon180/booking_monitor/pull/103)) + D12 (4-version comparison harness, [#104](https://github.com/Leon180/booking_monitor/pull/104) → [#109](https://github.com/Leon180/booking_monitor/pull/109); Stages 1-3 are new `cmd/booking-cli-stage{1,2,3}/` binaries, Stage 4 = the existing full-stack `cmd/booking-cli/`) + D14 README mermaid refresh ([#100](https://github.com/Leon180/booking_monitor/pull/100)) + D15 part 4 ([#101](https://github.com/Leon180/booking_monitor/pull/101); parts 1-3 shipped earlier in PRs [#81](https://github.com/Leon180/booking_monitor/pull/81)/[#82](https://github.com/Leon180/booking_monitor/pull/82)/[#83](https://github.com/Leon180/booking_monitor/pull/83) but were never credited in a release until now). v1.0.0 anchor was "D1-D16 complete" — D11 + D13 explicitly dropped during 2026-05-03 scope decision; everything else is shipped. See [CHANGELOG.md §1.0.0](../CHANGELOG.md) for the per-PR ledger. **Phase 3 portfolio arc closed.**
- **Post-v1.0.0 follow-ups** (not blocking): SLOWLOG + big-key observability (scheduled cloud routine ~2026-05-22), TT-Cache-1/4/5/6 (gates D8 multi-ticket-type expansion only), D4.3 dual-secret webhook rotation, O3.2 bare-metal benchmark. See [CHANGELOG.md §Unreleased](../CHANGELOG.md) for the parked list.
...
> **Replaced with:** D14 README architecture diagram (mermaid, ~1 day), D15 multi-post engineering blog series under `docs/blog/` using a hybrid-STAR template (~3-5 days, paced incrementally), D16 CHANGELOG.md + retroactive git tags + GitHub Releases page (~1 day, highest ROI of the three because it surfaces architecture-evolution at the GitHub repo entry). Asciinema terminal walkthrough lands as D10-minimal under "Minimal demo polish" rather than the portfolio-narrative section. Total replacement cost ~5-7 days vs ~7-10 weeks original.
  ...
  D4.2 ✅ **StripeGateway adapter** shipped (PR [#110](https://github.com/Leon180/booking_monitor/pull/110); merged 2026-05-09 → v1.0.0). `internal/infrastructure/payment/stripe_gateway.go` is a real `stripe-go v82.5.1` adapter behind the existing `domain.PaymentGateway` + `PaymentStatusReader` ports. Selection is config-driven via `PAYMENT_PROVIDER ∈ {mock, stripe}` (the actual env var name landed slightly different from the planned `PAYMENT_GATEWAY_MODE` — single binary mode switches between in-process mock + real SDK; no separate `stripe-mock` service needed because httptest impersonator harness in `stripe_gateway_test.go` covers offline CI). Builder pattern via `stripe.NewClient(key, WithBackends(...))` — no global SDK state mutation; multi-adapter-safe under `-race`.
       — Application layer NOT modified — proves Clean Architecture's gateway swap-in/swap-out (the only call site change was `PaymentStatusReader.GetStatus` interface widening from `(ctx, orderID)` to `(ctx, paymentIntentID string)`, threaded through `StuckCharging.PaymentIntentID` + reconciler null-guard via `strings.TrimSpace`).
       — Error taxonomy: 4 new `domain.ErrPayment*` sentinels (Declined / Transient / Misconfigured / Invalid), translated from `*stripe.Error` types via `mapStripeError` using `errors.Join` (preserves `errors.Is` on both sides). Stripe `Request-Id` captured + threaded into log fields.
       ...
       — Webhook verifier internals delegated to `stripewebhook.ValidatePayloadWithTolerance`; typed sentinels preserved via `mapStripeWebhookError` so the alert metric label taxonomy survives SDK upgrades.
       — Production-mode validation rejects `mock` provider, `*_test_*` keys, `pk_live_*` (publishable key), AND any non-production env where Stripe credentials are paired with `PAYMENT_PROVIDER=mock` (round-4 staging-mock-silent-money-void cross-field guard).
       — 4 rounds of multi-agent review (per-slice + pre-merge + post-open + post-smoke) actioned: 1 HIGH + 7 MEDIUM + 7 LOW. Live smoke against the user's Stripe test account validated end-to-end: real PaymentIntent minted in test dashboard, idempotency verified, signed webhook accepted, all metrics emitted.
       — PCI scope: out of SAQ A (Stripe Elements client-side; backend never receives raw card data). Runbook section [`docs/runbooks/README.md` § D4.2 cutover note](runbooks/README.md#d42-cutover-note--stripe-sdk-adapter-production) documents PCI scope statement + sandbox→live cutover checklist + webhook secret rotation playbook + leaked-key incident response.
