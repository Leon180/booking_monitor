...
| C4 | api | [internal/infrastructure/api/booking/handler.go](../../internal/infrastructure/api/booking/handler.go) (file moved from `api/handler.go` in PR #44) | Raw `err.Error()` returned in JSON body — leaks PG constraint names, driver errors, internal paths. | [#5](https://github.com/Leon180/booking_monitor/pull/5) |
...
| N4 | [internal/infrastructure/api/booking/handler_tracing.go](../../internal/infrastructure/api/booking/handler_tracing.go) (file moved from `api/handler_tracing.go` in PR #44) | `span.SetStatus(codes.Error, ...)` gated on `status >= 500` only — 4xx never reflected. | #6 |
