...
> English version: [2026-05-saga-pure-forward-recovery.md](2026-05-saga-pure-forward-recovery.md)
...
我們在 D1-D6 把 Pattern A 補上[1]:reservation TTL、payment intent、webhook 簽章驗證、過期 sweeper。D7 把 legacy A4 自動扣款路徑整個刪掉 — 800+ LOC、saga compensator 的生產觸發來源縮成 D5 webhook + D6 expiry sweeper(recon force-fail 仍是 stuck-charging 訂單在 gateway probe 沒解決時的少見 fallback)。
...
**對仍在 happy-path 分類的 execution,§5 三個條件在這個子集內成立**:

- **Save-points / retry**:每段 Tᵢ 都自帶 retry(worker PEL、Stripe webhook re-delivery);state 推進在每步都有持久化
- **沒有 abort-saga**:server 端沒有「取消這個 saga」這個命令;客戶的推進(或不推進)是外部觀察,不是 server 端內部下達
- **Retry 收斂**:idempotency-key + race-aware SQL 保證 retry 不重複扣款、不分裂 state。「provider 最終回 `succeeded`」是停留在這個分類的*前提*,不是我們能保證的事

退出這個分類的 execution(provider 拒絕、客戶棄單、TTL 過期)會經 D5(`payment_failed`)/ D6(`expired`)路由到 §4 backward recovery。**這些不是 §5 違反**,是同一篇 paper 描述的另一種 recovery 模式,我們對它跑 compensating transaction。

**結論**:對 happy-path 子集,完全不需要 compensating transaction。把這個子集放在 saga 系統裡是 architectural mismatch。
...
sequenceDiagram
    ...
    Note over WH: handleLateSuccess: MarkExpired<br/>→ ErrInvalidTransition(D6 已做過)<br/>→ re-read terminal、idempotent return<br/>(不重複 emit order.failed — D6 已 emit)<br/>+ payment_webhook_late_success_total{detected_at="sql_predicate"}<br/>+ 人工退款 alert
...
刪除 happy path 的 outbox 寫入是淨正向 — 兩段 SQL 變一段。Mockus et al. 2025 [10]\*(Meta industrial-scale 數據;arXiv preprint,未經 peer review)報告刪碼作為 engineering practice 跟 SEV-causing diff 顯著降低有相關性 — 方向上跟 D7 hot-path benchmark 觀察一致。

### 學術對齊

> *本節標 \* 的引用是來自我們 [companion 學習筆記](notes/2026-05-pattern-a-foundations.zh-TW.md) 的 second-hand synthesis,該筆記整合兩個 research agent 報告,只對三個來源做了 primary-source校對(Stripe PaymentIntent、Stripe Idempotency、Garcia-Molina 1987)。標 \* 的引用在寫稿時尚未對照原文驗證,請當作 orientation 看待,而非逐字權威。筆記檔追蹤每個來源的驗證狀態,日後補讀後會更新。*

- **Local compensation > global saga** — Psarakis et al. CIDR '25 [7]\*。second-hand synthesis 上這篇 paper 把 trade-off 描述為「local compensations reduce rollback cost; global sagas preserve consistency but delay convergence」。我們的 saga compensator 跑在 `app` 行程內(D7 後),就是 synthesis 描述的 local compensation
...
- **形式化保證** — Kløvedal et al. arXiv 2026 [12]\*(under OOPSLA review)。second-hand synthesis 上 Accompanist runtime 被描述為證明 saga atomicity 在 deterministic process / idempotent action / durable queue 三個假設下成立。如果 synthesis 是準確的,我們的三個系統(Go runtime / Kafka / PG outbox)會對應這三個假設
