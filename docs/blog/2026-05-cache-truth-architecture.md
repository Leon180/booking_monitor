...
**The one NOVEL item is the SETNX-on-rehydrate refinement.** It's a sound refinement of a standard pattern (rehydrate cache from source-of-truth), not an anti-pattern, for these reasons:

1. **The problem being solved**: rehydrate runs at app startup. If Redis already holds values (another instance is still running, the app restarted while a peer kept Redis alive, or new and old pods overlap during a rolling deployment), naive `SET` would clobber the live in-flight values — *quantities that have been Lua-deducted but not yet committed to DB by the worker*.
2. **What naive SET would cause**: SET would overwrite the counter with the DB's `available_tickets`. In-flight bookings that had been Lua-deducted but not yet committed get effectively "un-deducted" by the rehydrate — the result is **double-allocation** (the same ticket sold twice: once consumed by the in-flight booking, once given back out by the SET overwrite). This is a correctness bug, not a performance issue.
3. **Why SETNX is the right primitive**: the semantic exactly matches "set only if no existing value"; single round-trip, no GET-then-SET race window; native Redis command with well-documented behaviour, no custom Lua needed.
4. **Why this is a refinement, not a novel pattern**: the underlying pattern (rehydrate cache from source-of-truth) is industry standard. The SETNX-vs-SET choice is the standard pattern's edge-case handling — any engineer implementing rehydrate from first principles will arrive at the same answer. SETNX itself dates back to early Redis (now subsumed under `SET ... NX`); the usage is in no way innovative.
5. **Caveat**: SETNX preserves *whatever* existing value is there, including a wrong one. If Redis has corrupt leftover state (from another bug, or an operator manually running `SET event:{uuid}:qty 999`), SETNX preserves the corruption. The next drift-detector sweep covers this blind spot — the `cache_high` direction specifically detects the "Redis > DB" case. In other words: **rehydrate's job isn't to detect corruption; the drift detector's is**. The two mechanisms have separated responsibilities; neither's failure mode contaminates the other.
